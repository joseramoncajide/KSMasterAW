...
flights %>% 
  ...
  summarise(n =n(), n_carriers = n_distinct(carrier), total_time = sum(air_time))
...
#Encadenando summarise se van elimando la varible más a la derecha dentro de un group_by()
#Demo: número de vuelos por compañía por mes
# flights %>% 
#   filter(!is.na(arr_delay)) %>%
...
#rm(list = ls(pattern = "^kk")) 





fit = lm(formula = amount ~ city, data=kkk)
anova (fit)
...
#http://paulorenato.com/joomla/index.php?option=com_content&view=article&id=175:local-airport-departure-delay-analysis-with-r&catid=1:miscellaneous&Itemid=11
plot_data = flights %>%
  filter(origin == "JFK") %>%
  group_by(hour,origin)
plot_data = na.omit(plot_data)
b1<-ggplot(plot_data, aes(hour, arr_delay)) + ylim(0, 600) +
...
#carriers <- read.csv('data/L_UNIQUE_CARRIERS.csv') 
